...
class AnswersViewData(View):
	...
	def post(self, request):
		...
		if request['search']:
			qa = Answers.objects\
					.filter(Q(question_id=int(question)) &
							Q(answer_text__contains=request['search']))\
					.select_related('user').select_related('question')[(page-1)
															*limit:page*limit]
		else:
			qa = Answers.objects.filter(question_id=int(question))\
								.select_related('user')\
								.select_related('question')[(page-1)
															*limit:page*limit]
		...
		for q in qa:
			...
			one_a['foto'] = settings.MEDIA_URL+'avatars/'+\
							Profile.objects.get(user_id=q.user.id).photo
			...
			one_a['count_votes'] = VotesAnswers.objects.filter(answer=
															   q.id).count()
			data.append(one_a)
		...
		one_q['foto'] = settings.MEDIA_URL + Profile.objects.get(user_id=
											question_data[0].user.id).photo
...
class CorrectUncorrectAnswer(View):
	...
	def post(self, request):
		...
		if request['func'] == 'uncorrect':
			...
			# как правильный, пользователь в качестве правильного выбрал
			# другой и данный ответ (который в прошлом был правильный)
			# необходимо отенить его "правильность"
			try:
				change_answer_f = Answers.objects.select_related('question')\
									.get(question_id=request['id_q'],
										 is_correct=1)
