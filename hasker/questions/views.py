...
from hasker.accounts.models import Profile
...
class QuestionsView(View):
	...
	def post(self, request):
		#try:
			...
			# Получаем номер страницы, кол-во элементов на странице
			# и номер вопроса
			...
			if request['search'] and request['search'][0] != '#':
				qs = Question.objects.filter(Q(question_header__contains=
											   request['search'])
			...
			# Срабатывает, если поиск осуществляется по тэгам
				...
				qs = Question.objects.filter(tags__contains=
											 request['search'][1:])\
									.select_related('user')[(page-1)
															*limit:page*limit]
			else:
				qs = Question.objects.select_related('user')\
						.annotate(count=Count('votesquestion__id'))\
						.order_by('-count', '-pub_date')[(page-1)
														 *limit:page*limit]
			# Рендерим данные вопросов и собираем их в
			# "удобный" для шаблона вид
			all_data = {}
			data = []
			for q in qs:
				one_q = {}
				...
				if session_user:
					one_q['is_voted'] = VotesQuestion.objects\
						.filter(user_id=int(session_user),
								question_id=int(q.id)).count()
				# Если не голосовал, то даем ему возможность проголосовать
				else:
					one_q['is_voted'] = False
				# Подсчет количества ответов на вопрос
				# и голосов за данный вопрос
				one_q['count_votes'] = VotesQuestion.objects\
					.filter(question_id=q.id).count()
				one_q['count_answers'] = Answers.objects\
					.filter(question_id=q.id).count()
				# Создаем путь до фотографии пользователя, написавшего вопрос
				print(q.user.id)
				print('kk')
				one_q['foto'] = settings.MEDIA_URL+Profile.objects\
										.get(user_id=q.user.id).photo
				data.append(one_q)
			all_data['data'] = data
...
class MinusVote(View):
	...
	def post(self, request):
		try:
			...
			request_data = json.loads(request.body)
			with transaction.atomic():
				del_qs = VotesQuestion.objects.get(question_id=
												   request_data['id'],
									user_id=session_user)
				del_qs.delete()	
			return JsonResponse('true', safe=False)
